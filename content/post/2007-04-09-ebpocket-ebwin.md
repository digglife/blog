...
复制cab文件到PPC任意位置,单击安装.

### 6.注意事项.

  * 查词的时候可能需要选择不同的检索方式,比如前方,全文等等.当没有显示任何结果的时候可以多试几个,但是不推荐全文检索,因为,你可能会等到天荒地老&#8230;
  * 小学馆中日日中统合词典不支持汉字识别,查日文的时候只能用假名输入,汉日词典则只能用拼音输入.
  * &#8230;&#8230;(改进中)
...
EBwin是EBPocket的PC版本,大部分功能和EBPocket一样.

最新版本下载:[EBWin2.07下载(.exe 1.4MB)][11]

安装和使用:

  * 直接双击安装
  * 词典文件下载同EBPocket
  * 导入词典文件(即词典文件里的catalog文件 File－add dict)
